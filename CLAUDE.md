...
## 🚨 База даних — НЕБЕЗПЕЧНІ команди (інцидент 2026-05-22)

`prisma migrate diff --shadow-database-url <PROD_URL>` колись повністю стерла
production-БД (Prisma скидає будь-яку БД, передану як shadow). **НІКОЛИ:**
- `migrate diff --shadow-database-url` з будь-чим, крім локальної throwaway-БД
- `migrate reset`, `db push --force-reset/--accept-data-loss` на БД з даними
- raw `DROP DATABASE/SCHEMA`, `TRUNCATE` без підтвердження користувача

Безпечно й вільно: `prisma migrate deploy`, `prisma db push` (без деструктивних
прапорів), `migrate resolve --applied`. Migration-файли генерувати лише проти
локальної одноразової БД. Хук `~/.claude/hooks/db-guard.sh` блокує небезпечне.
