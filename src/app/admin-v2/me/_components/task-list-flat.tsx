      ...
      {loading && tasks.length === 0 ? (
        ...
        >
          ...
          <p className="text-sm font-semibold" style={{ color: T.textSecondary }}>
            {focus === "overdue"
              ? "Немає прострочених задач 🎉"
              : focus === "today"
                ? "Сьогодні нічого не горить"
                : focus === "next"
                  ? "Далі — порожньо"
          ...
          <p className="text-[11px] text-center max-w-xs">
            {focus === "overdue"
              ? "Усе зі строком виконується вчасно. Можна зосередитись на нових задачах."
              : focus === "today"
                ? "На сьогодні дедлайнів немає. Гарний час довести щось до кінця."
                : focus === "next"
                  ? "Задач з найближчими дедлайнами не знайшлось."
