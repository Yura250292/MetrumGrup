...
import {
  ...
  StageProgressSlider,
  StageDatesEditor,
} from "../stages-v2/_components/stage-actions-client";

/**
 * Тіло вкладки "Етапи" — рендериться inline у /[id]?tab=stages.
 * Раніше жило окремою сторінкою /stages-v2; тепер та сторінка → redirect.
 * Без auth/redirect (caller вже зробив) і без back-link/V2 PREVIEW
 * (вони не потрібні inline у tab-view).
 */
export async function ProjectStagesBody({
  id,
  session,
}: {
  id: string;
