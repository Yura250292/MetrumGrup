        ...
        {MOBILE_NAV.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="relative flex h-12 w-16 flex-col items-center justify-center gap-0.5 rounded-lg transition tap-highlight-none active:scale-95"
              style={{
            ...
            >
              ...
              <span className="truncate text-[11px] font-semibold leading-none max-w-[56px]">
                {item.label}
              </span>
              {badge > 0 && (
                <span
                  className="absolute right-2 top-1 inline-flex h-4 min-w-[16px] items-center justify-center rounded-full px-1 text-[10px] font-bold leading-none"
