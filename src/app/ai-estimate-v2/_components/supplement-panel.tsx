...
export function SupplementPanel({ controller }: { controller: AiEstimateController }) {
  ...
  const canSubmit =
    ...
    !!controller.savedEstimateId &&
    (controller.supplementFiles.length > 0 || controller.supplementInfo.trim().length > 0);

  return (
    <div
      className="fixed inset-y-0 right-0 z-40 flex w-[680px] flex-col"
    ...
    >
      ...
      >
        <div className="flex flex-col gap-1.5">
          ...
          <p className="max-w-[540px] text-xs leading-relaxed" style={{ color: T.textSecondary }}>
            Завантажте нові креслення, специфікації або зміни. AI зіллє їх у наявний кошторис.
          </p>
        </div>
        <button
      ...
      <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-8 py-6">
        {!controller.savedEstimateId && (
          ...
          >
            Збережіть кошторис у проєкт перед доповненням.
          </div>
        )}
        ...
        <div className="flex flex-col gap-2">
          ...
          <textarea
            ...
            placeholder="Опишіть зміни, які треба врахувати у кошторисі"
            rows={5}
      ...
      >
        ...
        <button
          onClick={controller.supplement}
          disabled={!canSubmit}
          className="flex items-center gap-2 rounded-xl px-5 py-3 text-sm font-bold text-white disabled:opacity-50"
          style={{ backgroundColor: T.accentPrimary }}
        >
          {controller.supplementing ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
          {controller.supplementing ? "Зливаємо…" : "Запустити злиття"}
        </button>
      </footer>
    </div>
  );
