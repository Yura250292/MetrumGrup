...
7. Якщо обговорювалась процедура — я зробив actionPlan з порядковими кроками?
8. Я витяг УСІ спецтерміни/абревіатури/системи в glossary з нормальними поясненнями?
9. suggestedTitle — у форматі «<Проєкт>. <Хто> про <Що>»?

Якщо щось «ні» — переробляй.

═══════════════════════════════════════════════════════════════════
ПОКРОКОВИЙ ПЛАН ДІЙ (actionPlan)
═══════════════════════════════════════════════════════════════════
Якщо в нараді обговорювали ПРОЦЕС/ЛАНЦЮЖОК ДІЙ де порядок має значення (бюрократичні процедури: спочатку отримати X, потім Y, потім подати Z) — формуй actionPlan як НУМЕРОВАНИЙ chronological sequence:
- title: коротке формулювання типу «Отримати назву обʼєкта у Лінецької»
- detail: 1-2 речення з конкретикою — у кого взяти, що зробити, на що прив'язати
- owner: відповідальна сторона якщо ясно («Метрум», «Замовник», «Прораб», «Юлія»)

Це окремо від tasks (одиничні задачі без жорсткого порядку). actionPlan показує саме процедуру: «Крок 1 → Крок 2 → Крок 3». Для нарад без процесу — лиши порожнім масивом.

═══════════════════════════════════════════════════════════════════
ГЛОСАРІЙ ТЕРМІНІВ (glossary)
═══════════════════════════════════════════════════════════════════
Знайди ВСІ спецтерміни, абревіатури, юридично-технічні поняття, назви процедур/документів/систем — поясни їх. Підсумок має бути самодостатнім: новий менеджер у компанії розуміє про що йшлося не гуглячи.

Що класти у glossary:
- Абревіатури: МУО, КЕП, ІПН, ПДВ, ФОП, ТОВ, КП-2в, RD-02 тощо
- Назви документів: «Договір генпідряду», «Наказ на технагляд», «Авторський нагляд», «Витяг із системи», «Повідомлення про початок підготовчих робіт»
- Спеціалізовані поняття: «Технічний нагляд», «Договірні ціни», «Підготовчі роботи»
- Зовнішні системи/інструменти: admin.e-construction.gov.ua, Прозорро, AnyDesk, 1С тощо
- Юридичні формулювання: «Містобудівні умови та обмеження», «Назва обʼєкта»

Формат елемента:
- term: коротке імʼя як воно звучить у нараді
- definition: 1-3 речення що це означає — загальна суть, не лише в межах цієї наради. Пиши так щоб читач без бекграунду зрозумів. Для абревіатур — спочатку розшифрування, потім пояснення.
- contextInMeeting: 1 речення про те ЯК термін фігурував у цій нараді (опц., null якщо нема цінності)

ВАЖЛИВО: спирайся на свої знання індустрії, а не лише на сказане. Якщо хтось згадав «КЕП» але не пояснив — ти все одно вкажи що це «кваліфікований електронний підпис, юридично значущий аналог власноручного». Не дублюй elementary речі (не пояснюй «Дзвінок», «Лист»). Тільки те що НЕ ОЧЕВИДНО для людини без бекграунду.

═══════════════════════════════════════════════════════════════════
SUGGESTED TITLE
═══════════════════════════════════════════════════════════════════
Формат: «<Проєкт/Контекст>. <Хто> про <Що>». Приклади:
- «АТБ-Гірник. Гейченко Юлія про підготовчі роботи»
- «Бюджет RD-02. Олег і Сергій — обговорення затвердження»
- «Постачальники. Аналіз структури боргу перед Будхатою»

Без префіксу «Нарада», без дати, лаконічно (5-9 слів).`;
    ...
    actionPlan: {
      ...
      items: {
        ...
        properties: {
          step: {
            type: "integer",
            description: "Порядковий номер кроку (1, 2, 3, ...)",
          },
          title: {
            type: "string",
            description:
              "Коротке формулювання дії (5-12 слів). Приклад: «Отримати назву обʼєкта у Лінецької».",
          },
          detail: {
            type: "string",
            description:
              "1-2 речення з конкретикою: у кого взяти, що зробити, на що прив'язати, які умови.",
          },
          owner: {
            type: ["string", "null"],
            description:
              "Відповідальна сторона якщо ясно: «Метрум», «Замовник», «Прораб», конкретне імʼя. null якщо не вказано.",
          },
        },
        required: ["step", "title", "detail", "owner"],
      },
      description:
        "Покроковий план — хронологічний ланцюжок дій де порядок має значення (бюрократичні процедури, послідовність документів). Якщо нарада не про процес — порожній масив.",
    },
    nextSteps: {
      type: "array",
      items: { type: "string" },
      description:
        "Наступні кроки — що відбудеться після цієї наради до наступного контрольного моменту. Це окремо від actionPlan: тут — короткострокові дії на найближчі 1-2 тижні.",
    },
    openQuestions: {
      type: "array",
      items: { type: "string" },
      ...
      items: {
        ...
        properties: {
          term: {
            type: "string",
            description:
              "Коротке імʼя терміну як воно звучить у нараді: «МУО», «КЕП», «Договір генпідряду», «admin.e-construction.gov.ua».",
          },
          definition: {
            type: "string",
            description:
              "1-3 речення що це означає у загальному сенсі (не лише в межах цієї наради). Для абревіатур — спершу розшифрування, потім пояснення. Пиши так щоб читач без бекграунду зрозумів.",
          },
          contextInMeeting: {
            type: ["string", "null"],
            description:
              "1 речення про те ЯК термін фігурував у цій нараді. null якщо нема додаткової цінності.",
          },
        },
        required: ["term", "definition", "contextInMeeting"],
      },
      description:
        "Глосарій спецтермінів/абревіатур/назв документів/систем, що згадуються у нараді. Спирайся на свої знання індустрії — пояснюй навіть якщо учасники не пояснювали. Не дублюй elementary речі. Якщо термінів нема — порожній масив.",
    },
  },
  required: [
    "suggestedTitle",
...
export async function POST(
  ...
  try {
    ...
    const userParts = [
      ...
      "6. ОБОВʼЯЗКОВО витягни ВСІ спецтерміни/абревіатури/назви систем у glossary[] з людськими поясненнями. МУО, КЕП, ІПН, назви документів, e-construction — не пропускай нічого що не є очевидним для людини без бекграунду.",
      "7. Імена/ПІБ використовуй ДОСЛІВНО як у транскрипті/ENTITIES — не перекладай і не адаптуй. «Любовь Николаевна» лишається «Любовь Николаевна».",
      "8. Зберігай усі цифри, суми, дати, обʼєкти.",
      "9. Якщо нарада довша 5 хв — summary має бути 8-15 речень, повноцінне.",
      "10. Самоперевірка: пройдись по всіх 9 пунктах із системного промпту перед відповіддю.",
    ]
      .filter((p) => p !== null)
      .join("\n");
