...
export async function GET(
  ...
  // Зразок: всі етапи проекту (включно з підетапами) + ВИТРАТИ (PLAN+FACT)
  // проекту без stageRecordId. Доходи свідомо виключені — вони не привʼязуються
  // до етапів. PROJECT_BUDGET (системні) теж виключені.
  ...
  if (entries.length === 0) {
    return NextResponse.json({
      data: { suggestions: [], stages, entries, proposedNewStages: [] },
      info: "Немає витрат для розподілу (доходи синхронізація ігнорує)",
    });
  }
  ...
  const systemPrompt = `Ти — досвідчений економіст-кошторисник будівельної компанії. Завдання: розподіли ВИТРАТИ (PLAN та FACT) проекту "${project.title}" по етапах виконання робіт. Вхід — лише витрати; доходи і PROJECT_BUDGET вже відфільтровано.

Думай покроково:
1) Прочитай кожен запис: title, description, category, kind (PLAN/FACT), сума, контрагент.
2) З назви роботи/матеріалу/категорії визнач етап будівельного циклу. Приклади:
   - "ескаватор для демонтажу", "вивіз сміття" → етап "Демонтаж"
   - "фарба", "валик", "грунтовка" → етап "Фарбування стін"
   - "плитка 1200×600", "клей цементний", "фуга" → етап "Облицювання плиткою"
   - "паливо для трактора" → підпідетап "Паливо" всередині "Робота техніки" всередині "Демонтаж"
3) Існуючі етапи проекту мають пріоритет: якщо запис очевидно відповідає існуючому → використай його stageRecordId.
4) Якщо існуючих етапів немає або не вистачає — СМІЛИВО створюй нові у proposedNewStages з осмисленими назвами. Не залишай запис без етапу, якщо назва дозволяє визначити роботу.
5) Допускається до 3 рівнів вкладеності (батько → дитина → онук). Структура з прикладу користувача:
   "Демонтаж" (root)
     └ "Робота техніки" (child)
         └ "Паливо" (grandchild)
     └ "Робота тракториста" (child)
   Групуй однотипні витрати у підетап, особливо коли є природна категоризація (матеріал / робота / техніка / паливо).
6) Типова послідовність будівельного циклу: Демонтаж → Підготовка майданчика → Фундамент → Каркас/Стіни → Перекриття → Дах → Інженерні мережі (Електрика, Сантехніка, Вентиляція) → Чорнове оздоблення → Чистове оздоблення → Здача обʼєкта.
...
Правила tempId: префікс "new-", унікальний. Якщо запис віднесено до нового стейджу — у suggestions поле stageRecordId = tempId цього стейджу. parentTempId може посилатись на інший tempId щоб створити вкладеність до 3 рівнів (root → child → grandchild).`;
  ...
  const proposedNewStages: ProposedStage[] = [];
  try {
