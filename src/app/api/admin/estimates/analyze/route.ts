...
`;
    ...
    // Явне wizardData.interiorOnly має пріоритет; інакше — евристика за obj'єктом.
    const interiorOnlyMode =
      typeof wizardData?.interiorOnly === 'boolean'
        ? wizardData.interiorOnly
        : wizardData?.objectType === 'apartment' ||
          wizardData?.objectType === 'office' ||
          wizardData?.workScope === 'renovation';

    const interiorModeNote = interiorOnlyMode
      ? `
## ⚠️ РЕЖИМ: ТІЛЬКИ ВНУТРІШНІ (ОЗДОБЛЮВАЛЬНІ) РОБОТИ

Це проєкт **інтер'єру вже збудованого об'єкта** (офіс / квартира / комерційне приміщення).
Аналізуй ВНУТРІШНЄ опорядження, НЕ фундамент/коробку/дах.

- "rooms" заповнюй фактичними приміщеннями об'єкта (для офісу — кабінети, конференц-зал,
  лобі, рецепція, серверна, архів, санвузли, кухня тощо), а не лише житловими.
- ОБОВ'ЯЗКОВО оціни: ГКЛ-перегородки (тип/товщина, 2 шари, зашивка колон та інсталяцій),
  скляні перегородки/конструкції, акустичні стельові панелі, влаштування стелі/підлоги,
  оздоблення стін, двері (у т.ч. скляні та технічні).
- Поля building.foundation / роботи з даху / фасаду — НЕ заповнюй (постав null або пропусти).
`
      : '';
