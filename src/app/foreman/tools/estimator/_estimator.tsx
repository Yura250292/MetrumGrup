...
import { overlapsExisting, placeAdjacent } from "@/lib/foreman/geometry";
import type { EstimatorState, Step } from "./_types";
...
const initialState: EstimatorState = {
  plan: { defaultCeilingHeight: DEFAULT_CEILING, rooms: [] },
...
function reducer(state: EstimatorState, action: Action): EstimatorState {
  switch (action.type) {
    ...
    case "SET_DEFAULT_CEILING":
      return {
        ...state,
        plan: { ...state.plan, defaultCeilingHeight: action.value },
      };
    case "ADD_ROOM":
      return {
        ...state,
        plan: { ...state.plan, rooms: [...state.plan.rooms, action.room] },
      };
    case "UPDATE_ROOM":
      return {
        ...state,
    ...
    case "REMOVE_ROOM": {
      ...
      // прибираємо пов'язану конфігурацію
      const works = { ...state.works };
      const w = { ...works.rooms };
      delete w[action.id];
      const tileSizes = { ...works.tileSizes };
      const thicknessCm = { ...works.thicknessCm };
...
export function Estimator({ firmId }: Props) {
  ...
  const handleAddFirst = (length: number, width: number, name: string, height: number) => {
    const room: Room = {
  ...
  const handleAttach = (
    ...
    if (!parent) return;
    const rect = placeAdjacent(parent, side, length, depth);
    ...
    const collided = overlapsExisting(rect, state.plan.rooms.filter((r) => r.id !== parent.id));
    if (collided) {
          ...
          >
            <PlanCanvas
              ...
              onPlusClick={(parentId, side) =>
                setSheet({ mode: "add", parentId, side })
              }
              onRoomTap={(room) => setSheet({ mode: "edit", room })}
            />
          </motion.div>
        )}
