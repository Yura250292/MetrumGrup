...
describe("computeCashflow — Phase 4.2 commitments + actualCash dimensions", () => {
  ...
  it("openingBalanceActualCash рахується тільки з ACTUAL_*", async () => {
    // Перший groupBy: всі FACT до from → 1000 income
    // Другий groupBy: тільки ACTUAL_* до from → 300 income
    groupByMock
      .mockResolvedValueOnce([
        { type: "INCOME", _sum: { amount: "1000" } },
      ] as never)
      .mockResolvedValueOnce([
        { type: "INCOME", _sum: { amount: "300" } },
      ] as never);
    findManyMock.mockResolvedValue([] as never);

    const r = await computeCashflow({ from, to, granularity: "DAY" });
    expect(r.openingBalance).toBe(1000);
    expect(r.openingBalanceActualCash).toBe(300);
  });
