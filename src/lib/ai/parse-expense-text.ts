...
Поверни ВИКЛЮЧНО валідний JSON-масив (без markdown-fence, без пояснень). Кожен елемент:
{
  ...
  "supplier": string або null  // постачальник/магазин/ФОП якщо явно згадано
}
...
ПРАВИЛО supplier (постачальник/магазин/ФОП):
- Якщо в тексті явно згадано магазин/фірму/ФОП — "купив у Будхаті", "взяв в Епіцентрі",
  "Епіцентр — цемент 500", "Накладна Будхата 12345" — заповни це поле назвою постачальника.
- Якщо є код ЄДРПОУ/РНОКПП — додай у дужках: "Будхата (12345678)".
- Якщо постачальник один на весь блок витрат — повторюй його значення для кожного item.
...
const ParsedExpenseSchema = z.object({
  ...
  supplier: z.string().trim().min(1).max(200).nullable().optional(),
});
...
Поверни ВИКЛЮЧНО валідний JSON-масив (без markdown-fence, без пояснень). Кожен елемент:
{
  ...
  "confidence": число 0..1,
  "rawLine": string,
  "supplier": string або null  // назва магазину/постачальника із шапки чека
}
...
ВАЖЛИВО: у шапці чека (зверху або знизу) типово є назва магазину/ФОП/ТОВ і код ЄДРПОУ/РНОКПП.
Витягни цю назву і повтори у кожен item у поле "supplier". Це критично для обліку боргів —
без supplier факт не привʼяжеться до постачальника.
