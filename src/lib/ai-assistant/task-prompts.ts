...
const META_RULE =
  "\n\nЯкщо задача стосується розробки самої платформи Metrum (CRM, адмінка, доступи, інтеграції) — секція PLATFORM_STATE у системному промпті містить актуальний стан модулів (🟢 робочі / 🟡 потребують доробки). НЕ давай generic поради на кшталт 'впровадити модуль X' якщо модуль вже 🟢. Замість цього називай конкретні прогалини або наступні кроки під реальний стан коду, з шляхами файлів/маршрутів. Перед загальним планом викликай get_task_list({ projectId }) щоб побачити чи частина пунктів вже у backlog'у — не дублюй їх.";
...
const ACTION_INSTRUCTIONS: Record<TaskAiAction, string> = {
  explain:
    `Коротко (3-5 речень) поясни суть цієї задачі з точки зору конкретних дій: що вона означає, чому вона важлива, що буде результатом.${META_RULE}`,
  breakdown:
    `Розбий цю задачу на 5-8 конкретних послідовних кроків. На кожному кроці: дія, очікуваний результат. Без води. Не вигадуй кроків які вже зроблені — спершу з'ясуй стан (для мета-задач про CRM див. PLATFORM_STATE, для будівельних — get_project_summary + get_task_list).${META_RULE}`,
  today:
    `Виходячи з поточного стану задачі (статус, чекліст, блокери) — що саме треба зробити сьогодні, щоб просунути її? 1-3 конкретні дії.${META_RULE}`,
  blockers:
    `Проаналізуй: що може блокувати цю задачу? Подивись на залежності, чекліст, статус, описання. Поверни короткий список потенційних блокерів і що з ними робити.${META_RULE}`,
  "who-to-involve":
  ...
  checklist:
    `Склади для цієї задачі чекліст з 5-10 конкретних пунктів у форматі '- [ ] крок'. Без загальних фраз, тільки дії.${META_RULE}`,
