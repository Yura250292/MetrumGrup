...
/**
 * Варіант секції "Стіни та конструкції" для режиму ВНУТРІШНІХ робіт.
 * Несучі конструкції/перекриття/колони вже існують — лишаються тільки
 * перегородки, ГКЛ-зашивки та підготовка стін під оздоблення.
 */
const INTERIOR_PARTITIONS_SECTION: SectionSpec = {
  title: 'Перегородки та ГКЛ-конструкції',
  description: 'Внутрішні перегородки, зашивки колон/інсталяцій, підготовка стін',
  required: true,
  minItems: 15,
  scope: [
    'Перегородки ГКЛ на металокаркасі (товщина 75/100/110/125 мм)',
    'Перегородки ГКЛ у 2 шари гіпсокартону',
    'Перегородки з газоблоку / пазогребеневих плит',
    'Зашивка колон ГКЛ у рівень з перегородкою',
    'Зашивка інсталяцій, стояків та комунікацій ГКЛ',
    'Нарощення / вирівнювання існуючих стін ГКЛ-конструкцією',
    'Звукоізоляція всередині перегородок (мінвата)',
    'Скляні перегородки та конструкції (каркас + скління)',
    'Армування кутів, стрічка-серпянка, шпаклівка стиків',
    'Штукатурка стін чорнова під оздоблення',
    'Підсилення стін під монтаж інсталяцій/обладнання',
  ],
};

/**
 * Варіант секції "Оздоблення" для режиму ВНУТРІШНІХ робіт —
 * без фасадних робіт і утеплення, з акустичними стельовими панелями.
 */
const INTERIOR_FINISHING_SECTION: SectionSpec = {
  title: 'Оздоблення',
  description: 'Чистове внутрішнє оздоблення: підлоги, стіни, стелі, двері',
  required: true,
  minItems: 25,
  useMiniModel: true,
  scope: [
    'Стяжка підлог (ЦПС / самовирівнювальна)',
    'Гідроізоляція підлог (санвузли)',
    'Наливна підлога',
    'Керамограніт / керамічна плитка для підлоги',
    'Клей для плитки, затирка швів',
    'Ламінат / паркетна дошка',
    'Плінтуси (МДФ / прихований)',
    'Пороги',
    'Штукатурка стін гіпсова',
    'Шпаклівка стартова + фінішна',
    'Грунтовка глибокого проникнення',
    "Фарбування стін інтер'єрною фарбою",
    'Шпалери / декоративна штукатурка',
    'Облицювання стін плиткою',
    'Підвісні стелі ГКЛ',
    'Стельові акустичні панелі',
    'Натяжні стелі',
    'Стельові плити Armstrong',
    'Двері міжкімнатні + коробки + лиштви',
    'Двері технічні / протипожежні металеві',
    'Відкоси та підвіконня',
    'Малярні витратні матеріали',
  ],
};

/**
 * Варіант секції "Електрика та IT" для ВНУТРІШНІХ робіт —
 * без блискавкозахисту та зовнішніх вводів (об'єкт уже під'єднано).
 */
const INTERIOR_ELECTRICAL_SECTION: SectionSpec = {
  title: 'Електрика та IT',
  description: 'Внутрішня електрика, освітлення, слабкострумові мережі',
  required: true,
  minItems: 18,
  scope: [
    'Поверховий/квартирний розподільчий щит',
    'Силові кабелі всередині (ВВГнг 3х2.5, 3х4, 5х6 мм²)',
    'Кабель в гофрі/лотках, штроблення',
    'Автоматичні вимикачі, УЗО (ПЗВ)',
    'Розетки (звичайні, вологозахищені, силові)',
    'Вимикачі',
    'Освітлення LED (панелі, світильники, трекові)',
    'Аварійне освітлення + знаки EXIT',
    'Слабкострум — кабель UTP Cat6, оптоволокно',
    'IT-шафа / серверна стійка',
    'Відеонагляд (камери + реєстратор + кабелі)',
    'Контроль доступу (зчитувачі, електрозамки)',
    'Пожежна сигналізація (сповіщувачі, шлейфи, ППКП)',
    'Контур заземлення в межах приміщень',
  ],
};

/**
 * Варіант секції "Сантехніка" для ВНУТРІШНІХ робіт —
 * тільки внутрішні мережі та прилади, БЕЗ вводів, колодязів і
 * зовнішньої/дощової каналізації.
 */
const INTERIOR_PLUMBING_SECTION: SectionSpec = {
  title: 'Сантехніка',
  description: 'Внутрішнє водопостачання, каналізація, сантехприлади',
  required: true,
  minItems: 12,
  useMiniModel: true,
  scope: [
    'Водопровідні труби всередині (ПП/PEX) + ізоляція',
    'Фітинги, запірна арматура, колектори',
    'Розводка каналізації всередині (ПВХ 50/110 мм)',
    'Ревізії, трапи',
    'Інсталяції для унітазів',
    'Унітази з бачком',
    'Раковини + тумби/стільниці',
    'Душові кабіни / піддони (якщо є)',
    'Змішувачі',
    'Водонагрівач (бойлер), якщо передбачено',
    'Гідроізоляція санвузлів',
    'Підключення сантехприладів',
  ],
};

/**
 * Варіант секції "Вентиляція та опалення" для ВНУТРІШНІХ робіт —
 * офісна інженерія, БЕЗ котельні, промислового холоду й торгового обладнання.
 */
const INTERIOR_HVAC_SECTION: SectionSpec = {
  title: 'Вентиляція та опалення (HVAC)',
  description: 'Внутрішня вентиляція, кондиціонування, опалення офісу',
  required: true,
  minItems: 14,
  useMiniModel: true,
  scope: [
    'Припливно-витяжна установка з рекуператором',
    'Повітроводи оцинковані (круглі/прямокутні) + фасонина',
    'Ізоляція повітроводів (K-flex)',
    'Дифузори, решітки припливні/витяжні',
    'Вогнезатримуючі клапани',
    'Кондиціонери: спліт / мультиспліт / VRF (внутрішні + зовнішні блоки)',
    'Мідні фреонові траси + ізоляція, дренаж',
    'Радіатори / конвектори опалення + кріплення',
    'Труби опалення (PEX/метал) у межах приміщень',
    'Терморегулятори, балансування',
    'Тепла підлога (за наявності) — мати, колектор',
    'Автоматика та пусконалагодження систем',
  ],
};
...
export class MasterEstimateAgent {
  ...
  private selectSections(context: AgentContext): SectionSpec[] {
    ...
    // Перемикач "внутрішні роботи vs будівництво з нуля".
    // Явне wizardData.interiorOnly має пріоритет; якщо не задано —
    // визначаємо евристично за obj'єктом/обсягом робіт.
    const interiorOnly =
    ...
    if (interiorOnly) {
      // Об'єкт уже збудований — прибираємо майданчик / коробку / дах
      sections = sections.filter(s =>
        !['Земляні роботи', 'Фундамент', 'Покрівля'].includes(s.title)
      );
      // Підміна секцій на внутрішні варіанти (без зовнішніх мереж/фасаду):
      //  Стіни → перегородки/ГКЛ; Оздоблення → без фасаду;
      //  Електрика → без блискавкозахисту/вводів; Сантехніка → без колодязів.
      sections = sections.map(s => {
        if (s.title === 'Стіни та конструкції') return INTERIOR_PARTITIONS_SECTION;
        if (s.title === 'Оздоблення') return INTERIOR_FINISHING_SECTION;
        if (s.title === 'Електрика та IT') return INTERIOR_ELECTRICAL_SECTION;
        if (s.title === 'Сантехніка та каналізація') return INTERIOR_PLUMBING_SECTION;
        if (s.title === 'Вентиляція та опалення (HVAC)') return INTERIOR_HVAC_SECTION;
        return s;
      });
      console.log('📋 [selectSections] Режим: ВНУТРІШНІ роботи (без фундаменту/коробки/покрівлі/фасаду)');
    } else {
      console.log('📋 [selectSections] Режим: повний цикл (будівництво з нуля)');
    }
...
Якщо точних площ приміщень немає в обмірі — познач припущення в notes.`
    ...
    const knuVolumeHints: Record<string, string> = {
      ...
      'Перегородки та ГКЛ-конструкції': 'Збірники 8, 9, 15 — перегородки, ГКЛ, зашивки',
      ...
      'Сантехніка': 'Збірники 16, 17 — 267 норм (внутрішні мережі)',
...
- Якщо матеріал самостійний (без прив'язки до конкретної роботи у цій секції) — parentSortOrder: null.
- Робота (itemType="work") ЗАВЖДИ має parentSortOrder: null.
- ПОРЯДОК: спочатку у списку йде робота, відразу за нею — її матеріали. Тобто якщо "Влаштування огорожі" має sortOrder=1, її матеріали (Профнастил, Стовпці, Лаги, Саморізи) йдуть як items[2..5] з parentSortOrder=1.
...
- НЕ дублюй позиції з інших секцій (див. "Вже згенеровані секції" у промті)`;
  ...
  private buildSectionPrompt(
    ...
    // Візуальний обмір креслень — ПРІОРИТЕТНЕ джерело кількостей.
    // Додаємо у КОЖНУ секцію (compactMasterContext його обрізає), бо площі/
    // довжини/кількості потрібні для quantity у всіх розділах.
    if (context.documents?.drawingsVisual) {
      prompt += `${context.documents.drawingsVisual}\n\n`;
      prompt += `🔒 ОБМІР — ОБОВ'ЯЗКОВЕ ДЖЕРЕЛО КІЛЬКОСТЕЙ:\n`;
      prompt += `1. Якщо площа/довжина/кількість для позиції Є в обмірі вище —\n`;
      prompt += `   став quantity ТОЧНО це значення. НЕ округлюй «на око», НЕ вигадуй інше.\n`;
      prompt += `2. Якщо в обмірі є перелік приміщень з площами — підлога/стеля/стіни\n`;
      prompt += `   рахуються від цих площ, а не від загальної площі проєкту.\n`;
      prompt += `3. Матеріали/марки бери з розділу «СПЕЦИФІКАЦІЇ» обміру, якщо він є.\n`;
      prompt += `4. Тільки якщо даних у обмірі НЕМА — оцінюй, і додай у notes "оцінка (немає в обмірі)".\n`;
      prompt += `5. У notes кожної позиції з обміру коротко зазнач джерело, напр. "обмір: Кабінет 40,8 м²".\n\n`;
    }
    ...
    // 🔑 Режим кошторису: внутрішні роботи vs будівництво з нуля
    const interiorOnlyMode =
    ...
    if (interiorOnlyMode) {
      prompt += `\n## ⚠️ РЕЖИМ: ТІЛЬКИ ВНУТРІШНІ (ОЗДОБЛЮВАЛЬНІ) РОБОТИ\n`;
      prompt += `Об'єкт уже збудований — рахуємо ВНУТРІШНЄ опорядження діючої будівлі.\n`;
      prompt += `❌ НЕ додавай: земляні роботи, фундамент, несучі стіни/колони/перекриття,\n`;
      prompt += `   покрівлю, фасад, утеплення фасаду, благоустрій території.\n`;
      prompt += `❌ НЕ додавай ЗОВНІШНІ мережі: ввід водопроводу, водопровідні/каналізаційні\n`;
      prompt += `   колодязі, зовнішню/дощову каналізацію, жироуловлювачі, блискавкозахист.\n`;
      prompt += `✅ Тільки внутрішні мережі в межах приміщень: труби/кабелі всередині,\n`;
      prompt += `   щит, прилади, перегородки, оздоблення, двері — у межах цієї секції.\n\n`;
    }
  ...
  private enrichWithLaborSources(sections: EstimateSection[]): EstimateSection[] {
    ...
    const result = sections.map(section => {
      const updatedItems = section.items.map(item => {
        ...
        // Якщо AI явно позначив позицію як матеріал — пропускаємо labor enrichment
        // (нова канонічна семантика; AI сам визначає тип у промпті).
        if (item.itemType === 'material' && (item.laborCost ?? 0) === 0) {
          return item;
        }

        // Fallback heuristic для випадків коли AI не передав itemType (legacy/edge cases):
        // детектуємо чистий матеріал по опису без дієслова монтажу.
      ...
      const newSectionTotal = updatedItems.reduce((sum, i) => sum + i.totalCost, 0);
