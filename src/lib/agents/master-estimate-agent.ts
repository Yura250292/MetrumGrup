...
export class MasterEstimateAgent {
  ...
  private selectSections(context: AgentContext): SectionSpec[] {
    ...
    if (interiorOnly) {
      ...
      // "Стіни та конструкції" → тільки перегородки/ГКЛ;
      // "Оздоблення" → внутрішнє, без фасаду
...
- НЕ дублюй позиції з інших секцій (див. "Вже згенеровані секції" у промті)`;
  ...
  private buildSectionPrompt(
    ...
    if (interiorOnlyMode) {
      ...
      prompt += `✅ Додавай ТІЛЬКИ: перегородки та ГКЛ-конструкції, зашивки колон/інсталяцій,\n`;
      prompt += `   електрику, сантехніку, ОВ, оздоблення підлог/стін/стель, двері — у межах цієї секції.\n\n`;
    }
